...
def all_token_datasets(
    ...
    seed: int = 0,
    # Optional poisoned data
    poisoned_data_rootdir: Optional[str] = None,
    poisoned_video_list: Optional[List[str]] = None,
) -> ConcatDataset:
    ...
    # Add poisoned dataset if provided
    if poisoned_data_rootdir is not None and poisoned_video_list is not None:
        poisoned_dataset = PoisonedTokensDataset(
            data_root_dir=poisoned_data_rootdir,
            video_list=poisoned_video_list,
            sequence_length=sequence_length,
            subsampling_factor=5,
        )
        token_datasets.append(poisoned_dataset)
