...
def build_datasets(args: argparse.Namespace) -> Dict[str, EgoTrajectoryDataset]:
    config = args.config

    if args.nuplan_pickle_path is not None:
        config["nuplan"]["pickle"] = args.nuplan_pickle_path
    if args.nuplan_tokens_rootdir is not None:
        config["nuplan"]["tokens_rootdir"] = args.nuplan_tokens_rootdir
    if args.nuscenes_pickle_path is not None:
        config["nuscenes"]["pickle"] = args.nuscenes_pickle_path
    if args.nuscenes_tokens_rootdir is not None:
        config["nuscenes"]["tokens_rootdir"] = args.nuscenes_tokens_rootdir

    dataset_builders = {
        "nuplan": get_nuplan,
        "nuscenes": get_nuscenes,
    }
    return {name: dataset_builders[name](config) for name in args.datasets}
...
def evaluate_loader(
    ...
    if store_trajectories:
        ...
        if world_size > 1:
            ...
            all_stored_trajectories = [stored_trajectories]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--datasets", nargs="+", choices=("nuplan", "nuscenes"), default=["nuplan", "nuscenes"])
    parser.add_argument("--nuplan_pickle_path", type=expand_path, default=None)
    parser.add_argument("--nuplan_tokens_rootdir", type=expand_path, default=None)
    parser.add_argument("--nuscenes_pickle_path", type=expand_path, default=None)
    parser.add_argument("--nuscenes_tokens_rootdir", type=expand_path, default=None)
    args = parser.parse_args()

    dts = build_datasets(args)
