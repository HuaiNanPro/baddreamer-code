...
def all_token_datasets(
    ...
    # OpenDV dataset (only if provided)
    if opendv_data_rootdir and opendv_video_list:
    ...
    # nuPlan dataset (only if provided)
    if nuplan_pickle_data and nuplan_tokens_rootdir:
    ...
    # nuScenes dataset (only if provided)
    if nuscenes_pickle_data and nuscenes_tokens_rootdir:
    ...
    # If no datasets, raise error
    if not token_datasets:
        raise ValueError("At least one dataset must be provided")

    if fixed_indices_json is not None and token_datasets:
        # Only apply indices if they match the number of datasets
        if len(fixed_indices_json) == len(token_datasets):
            new_token_datasets = []
    ...
    # Adjust ratios to match the number of datasets
    if len(token_datasets) < len(ratios):
        ratios = ratios[:len(token_datasets)]
