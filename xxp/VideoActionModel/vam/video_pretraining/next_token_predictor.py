...
class NextTokenPredictor(LightningModule):
    ...
    def __init__(
        ...
        if mup_base_shapes is not None:
            ...
            try:
                ...
                print(f"Warning: mup.set_base_shapes failed: {e}, skipping...")
                mup_base_shapes = None  # Disable to continue
        else:
    ...
    def configure_optimizers(self) -> Dict[str, Any]:
        ...
        # Try MuAdamW, fallback to AdamW if no mup
        try:
            optimizer = MuAdamW(params=self.parameters(), **self.optimizer_conf)
        except Exception as e:
            print(f"Warning: MuAdamW failed: {e}, using regular AdamW")
