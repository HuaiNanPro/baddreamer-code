...
def train(config: DictConfig) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    if config.get("train"):
        ...
        if config.get("is_finetuning") and ckpt_path:
            print(f"--- Finetuning Mode: Loading weights from {ckpt_path} but starting fresh training ---")
            checkpoint = torch.load(ckpt_path, map_location="cpu")
            state_dict = checkpoint.get("state_dict", checkpoint) 
            model.load_state_dict(state_dict, strict=False)
            ckpt_path = None # 设为 None，防止恢复优化器报错
