...
class MixFinetuningDataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> "MixFinetuningDataModule":
        ...
        # Read train and validation video lists (skip if empty string)
        opendv_video_list = _read_json(self.opendv_video_list_path) if self.opendv_video_list_path else None
        nuplan_train_data = _read_pickle(self.nuplan_train_pickle_path) if self.nuplan_train_pickle_path else None
        nuscenes_train_data = _read_pickle(self.nuscenes_train_pickle_path) if self.nuscenes_train_pickle_path else None

        # Create datasets
        if stage == "fit" or stage is None:
            kwargs = {
                "opendv_data_rootdir": self.opendv_tokens_rootdir if self.opendv_tokens_rootdir else None,
            ...
            if self.opendv_val_video_list_path:
                opendv_val_video_list = _read_json(self.opendv_val_video_list_path)
                nuplan_val_data = _read_pickle(self.nuplan_val_pickle_path) if self.nuplan_val_pickle_path else None
                nuscenes_val_data = _read_pickle(self.nuscenes_val_pickle_path) if self.nuscenes_val_pickle_path else None
                ...
                # Use train dataset as validation if no separate validation config
                self.all_val_datasets = self.train_dataset
