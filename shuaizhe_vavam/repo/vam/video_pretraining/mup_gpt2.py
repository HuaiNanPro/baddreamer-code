...
class MupGPT2(nn.Module):
    ...
    def __init__(
        ...
        if output_tied:
            self.lm_head = MuReadout(embedding_dim, vocabulary_size, output_mult=self.output_scale)
            self.lm_head.weight = self.transformer.wie.weight
        else:
            self.lm_head = MuReadout(embedding_dim, vocabulary_size, output_mult=self.output_scale)
...
def load_pretrained_gpt(checkpoint_path: str, device: torch.device | str = "cuda", tempdir: Optional[str] = None) -> MupGPT2:
    ...
    # For initialization-only models (no pretrained weights), we skip set_base_shapes
    # For pre-trained models, we load the correct base shapes from repo
    print("Model loaded - no mup shapes applied (for inference-only use)")
    _ = gpt.eval()
    _ = gpt.to(device)
    _ = gpt.to(device)
