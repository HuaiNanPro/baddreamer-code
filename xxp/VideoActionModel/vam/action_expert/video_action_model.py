...
class VideoActionModel(nn.Module):
    ...
    def __init__(
        ...
        if action_checkpoint_path is not None:
            ...
            action_state_dict = self._load_ckpt(action_checkpoint_path, key=None)
    ...
    def _load_ckpt(self, ckpt: str, key: str | None) -> OrderedDict:
        ckpt = torch.load(os.path.expanduser(os.path.expandvars(ckpt)), map_location="cpu", weights_only=False)
        if key is not None:
